...
public class MazeGraphics extends JPanel {
     ...
     * @param xStart   The first x coordinate to be filled in (first y coordinate is
     *                 0)
     */
    public MazeGraphics(char[][] maze, ArrayList<String> routes, int startRow, int startCol, String message,
            int xStart) {
        this.maze = maze;
        this.routes = routes;
    ...
    public void drawMaze() {
        ...
        frame.setSize((maze[0].length + 1) * 50, (maze.length + 1) * 50); // Sets the size of the frame to fit the size
                                                                          // of the array
        JPanel gPanel = new JPanel();
        gPanel.add(this);
        ScrollPane scroller = new ScrollPane(ScrollPane.SCROLLBARS_AS_NEEDED); // Scroller so we can scroll through the
                                                                               // mazes
        scroller.add(gPanel);
     ...
     * 
     * @param g The graphics involved with the paintComponent() method of the JPanel
     *          class
     */
    public void paintComponent(Graphics g) {
        ...
        for (m = 0; m < routes.size(); m++) {
            ...
            // Goes through the route and adds Xs to each of the points on the maze that
            // have been visited for this route
            ...
            // Changing the color of the paint component according to the position on the
            // maze
            for (i = 0; i < maze.length; i++) {
                for (j = 0; j < maze[0].length; j++) {
                    if (maze[i][j] == '|') {
