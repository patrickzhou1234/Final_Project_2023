...
public class Maze {
    ...
    /**
     * Default Constructor of the Maze class which initializes all the instance variables to a default value
     * 
     * @return none
     */
    public Maze() {
    ...
    /**
     * Constructor of the Maze class which initializes the instance variables based on the parameters
     * 
    ...
    /**
     * Finds all possible routes to get out of the maze from the starting point, which 
     * is the value of currentRow and currentCol in the first call of the method. 
     * Each route is a string which can contain "N", "S", "E", or "W" which would be the different
     * directions you from the starting point. All of these routes are stored in the routes ArrayList.
     * 
     * @param currentRoute The current route from the starting point, which is what you have moved so far
     * @param currentRow The row of your current position
     * @param currentCol The column of your current position
     * @param usedCoords ArrayList consisting of coords which you have already visited. (Each coord is row + " " + column)
     * 
     * @return none
     */
    private void calculateRoutes(String currentRoute, int currentRow, int currentCol, ArrayList<String> usedCoords) {
        ...
        // Checks if you are at the edge of the maze (which means you have reached an exit)
        if (currentRow == 0 || currentCol == 0 || currentRow == maze.length - 1 || currentCol == maze[0].length - 1) {
            routes.add(currentRoute);
            return;
        }

        // Checks for each point around your current coordinates to see if it is available, and calls the method again with those coords if that point is available
        if (maze[currentRow][currentCol - 1] == ' ') {
            if (!coordUsed((currentRow) + " " + (currentCol - 1), usedCoords)) {
                newUsedCoords = new ArrayList<String>(usedCoords); // Copies usedCoords ArrayList in a new ArrayList to make sure the usedCoords is only updated for this call to currentRoute()
    ...
    public boolean coordUsed(String target, ArrayList<String> usedCoords) {
        for (String coords : usedCoords) { // Linear search, since it is not sorted in ascending order, so we can't do binary search
    ...
    public ArrayList<String> getShortestRoute() {
        ...
        // Loops through each route in the routes ArrayList and checks if its length is minimum than the current minimum amount
        if(routes.size() >= 1){
